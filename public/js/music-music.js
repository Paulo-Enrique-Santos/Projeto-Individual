...
function atualizarMusicas(){
    fetch("/atualizar/atualizarMusicasMusica", {
        method: "POST",
        headers: {
        ...
        body: JSON.stringify({
            ...
            idPlaylistServer: sessionStorage.MUSICA_IDMUSICA
        })
        ...
        if (resposta.ok) {
            resposta.json().then(function (resposta) {
                ...
                for (let i = 0; i < resposta.length; i++) {
                    ...
                    var fotoMusica = document.getElementById(`foto_Musica`);
                    var nomeMusica = document.getElementById(`nome_Musica`);
                    var nomeArtista = document.getElementById(`nome_Artista`);

                    if (verMusica != null) {
                        artist.innerHTML += ', ' + resp.artista;
                        nomeArtista.innerHTML += ', ' + resp.artista;
                    } else {

                        fotoMusica.style.backgroundImage = `url("/assets/picture/${resp.caminhoFoto}")`;
                        nomeMusica.innerHTML = `${resp.musica}`;
                        nomeArtista.innerHTML = `${resp.artista}`;
            ...
            throw ("Houve um erro ao listar as musicas!");
        }
